...
var Navigate = (function (obj) {
  ...
  function getMarkers(lat, lon) {
    _this.markers = [];
    $.getJSON('http://www.happstr.com/api/checkins?lat='+lat+'&lon='+lon+'&range=100000',
              function(msg) {
...
var HappyProcess = (function (obj) {
  ...
  function postHappy(lat, lon) {
    $.post('http://www.happstr.com/api/checkins',
      function(data) {
  ...
  function postBecause(because) {
    if(_this.checkinID) {
      $.ajax({
        url: 'http://www.happstr.com/api/checkins/' + _this.checkinID,
        type: "PUT",
        data: {
          comment: because
        },
